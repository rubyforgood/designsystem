...
const { signIn, BASE, RUNS, PRIMARY } = require("./targets");
...
async function ensureRail(page) {
  ...
  // No `.table-scroll` at all means this app has no scroll-rail component to self-test against --
  // a structural absence, not the "freshly seeded database" data-absence the comment above already
  // handles. Fail with a clear, specific message so the caller can skip the control instead of the
  // whole script dying on an opaque Playwright timeout five controls later.
  if (!hasRegion) throw new Error("no .table-scroll on this page -- no rail to self-test");
...
(async () => {
  ...
  for (const control of CONTROLS) {
    ...
    // This whole file exercises the *checks*, against fixtures (a scroll rail, a help link, a
    // frozen column) this app builds for itself. An app that doesn't have one of those fixtures
    // fails a `mutate`/`run` step with a clear error (see ensureRail), and that's a fact about the
    // app under test, not a bug in the check -- so it's a skip, not a crash or a FAIL.
    try {
      /*
      ...
      skipped.push(control);
      console.log(`  SKIP ${control.check.padEnd(7)} ${control.kind.padEnd(8)} ` +
        `${control.what} -- ${e.message.slice(0, 60)}`);
      continue;
    }
  ...
  console.log(`\n${ran} controls run over ${checks.size} checks, ${wrong.length} wrong, ` +
    `${skipped.length} skipped (fixture not present on this app)`);
  if (!wrong.length && ran) {
    console.log("every check that ran fires on a real defect and stays quiet on a benign one");
  }
